                ...
                for dir in range(4):
                    for dir_ in range(4):
                        if dir==dir_:
                            continue
                        if (dir+2)%4==dir_:
                            g[i*N*4+j*4+dir].append((i*N*4+j*4+dir_, 0.1))
                        else:
    ...
    for i in range(N):
        for j in range(N-1):
            if C[i][j]!=0 and C[i][j]==C[i][j+1]:
                game.connect(i, j, i, j+1)
    for i in range(N-1):
        for j in range(N):
            if C[i][j]!=0 and C[i][j]==C[i+1][j]:
                game.connect(i, j, i+1, j)
...
def solve(N: int, K: int, C: List[int]):
    ...
    while time()-TS<=2.0:
        if cnt>=50:
            break
        i, j = random.choice(list(not_target))
        diff = random.choice(DIR)
