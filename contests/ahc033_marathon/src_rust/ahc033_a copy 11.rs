...
lazy_static!{
    ...
    // 最短経路の候補
    static ref MINDIST_CANDIDATE3: Vec<Vec<usize>> = vec![
        vec![5, 1, 2], vec![], vec![5, 2, 1, 3], vec![], vec![5, 3, 1, 2], vec![], vec![5, 2, 1, 3], vec![], vec![3, 5, 2],
        vec![], vec![], vec![], vec![], vec![], vec![], vec![], vec![], vec![],
        vec![5, 0, 1, 2], vec![], vec![5, 2, 1, 3, 0], vec![], vec![5, 0, 3, 1, 2], vec![], vec![5, 2, 1, 3, 0], vec![], vec![0, 3, 5, 2],
        vec![], vec![], vec![], vec![], vec![], vec![], vec![], vec![], vec![],
...
impl State{
    fn init_state(input: &Input) -> Self{
        ...
        for perm in (0..SIZE).permutations(SIZE){
            ...
            tmp_order.append(&mut rest_order);
            let mut state = Self {place_task_order: tmp_order.clone(), out_task_wait: out_task_wait.clone(), score: None};
            let (_, max_used_space_cnt, undesired_cnt) = state.get_order(input);
            if max_used_space_cnt*1000+undesired_cnt<best_score{
                best_score = max_used_space_cnt*1000+undesired_cnt;
                place_task_order = tmp_order;
            }
        }
        Self {place_task_order, out_task_wait, score: None}
    }
    ...
    fn get_order(&self, input: &Input) -> (Vec<usize>, usize, usize){
        ...
        let mut on_out_queue = [false; SIZE*SIZE];
        ...
        for i in 0..SIZE{
            ...
            if next_places[i].last().unwrap()%SIZE==0{
                out_queue.push((Reverse(0), input.A[i][0], i));
                on_out_queue[input.A[i][0]] = true;
            }
        }
        ...
        while outed.len()<SIZE*SIZE{
            ...
            turn += 1;
            // やっていいout
            if let Some((Reverse(turn_request), item, row)) = out_queue.pop(){
                if turn_request<=turn{
                    ...
                    out_queue.push((Reverse(turn_request), item, row));
                }
            }
            // place
            if on_spaces.len()<7{
                if let Some((row, item)) = place_order.pop(){
                    if !outed.contains(&item){
                        if on_out_queue[item]{
                            // outしちゃう
                            out_queue.retain(|x| x.1!=item);
                            if !on_spaces.remove(&item){
                                next_places[row].pop();
                                if let Some(next_item) = next_places[row].last(){
                                    if next_outs[next_item/SIZE]==*next_item{
    ...
    fn update(&mut self, params: &Neighbor){
        if params.mode==0{
            self.place_task_order.swap(params.i, params.j);
        } else if params.mode==1 {
            let v = self.place_task_order.remove(params.i);
            self.place_task_order.insert(params.j, v);
        } else {
            if params.j==0{
                self.out_task_wait[params.i] -= 1;
            } else {
                self.out_task_wait[params.i] += 1;
            }
    ...
    fn get_neighbor(&self, input: &Input) -> Neighbor{
        ...
        if mode_flg<40{
            ...
            let mut j: usize;
            loop{
                i = rng.gen::<usize>()%self.place_task_order.len();
                j = rng.gen::<usize>()%self.place_task_order.len();
                if self.place_task_order[i]!=self.place_task_order[j]{
                    break;
                }
            }
            ...
            let mut j: usize;
            loop{
                i = rng.gen::<usize>()%self.out_task_wait.len();
                j = rng.gen::<usize>()%2;
                // if i%SIZE==0{
                //     continue;
                // }
                if j==0 && self.out_task_wait[i]>0{
                    break;
                }
                if j==1 && self.out_task_wait[i]<10{
                    break;
                }
            }
            Neighbor{i, j, score: self.score, mode: 2}
        }
    }

    fn simulate(&self, input: &Input, order: &Vec<usize>) -> (Vec<Crane>, usize, usize){
        let mut task_assigner = TaskAssigner::new(input, order);
        let mut cranes = vec![
    ...
    fn cal_score(&mut self, input: &Input, order2score: &mut HashMap<Vec<usize>, f64>) -> f64{
        if let Some(score) = self.score{
            return score;
        }
        let order = self.get_order(input).0;
        if let Some(&score) = order2score.get(&order){
            self.score = Some(score);
            return score;
        }
        let (cranes, out_cnt, rest_place) = self.simulate(input, &order);
        let score = cranes.iter().map(|c| c.ops.len()).max().unwrap() as f64 + (SIZE*SIZE-out_cnt) as f64 * 1000.0 + rest_place as f64;
        order2score.insert(order, score);
        self.score = Some(score);
        score
    }

    fn print(&self, input: &Input){
        let (cranes, out_cnt, rest_place) = self.simulate(input, &self.get_order(input).0);
    ...
    fn get_min_dist_to_space(&self, fr: usize, after: usize, consider_placed_container: bool) -> Vec<usize>{
        ...
        while !q.is_empty(){
            ...
            if spaces.contains(&(u%81)) && u/81>=self.container_map[u%81].last().unwrap().1 && u/81>=self.should_place_after[u%81] && !self.crane_map[u+162]{
                ...
                let mut path = vec![4];
                let mut v = u;
                while let Some(&u_op) = backtrace.get(&v){
                    path.push(u_op%10);
                    v = u_op/10;
                }
                path.reverse();
...
fn simanneal(input: &Input, init_state: State, timer: &Instant, limit: f64) -> State{
    let mut order2score = HashMap::new();
    let mut state = init_state.clone();
    let mut best_state = init_state.clone();
    let mut best_score = best_state.cal_score(input, &mut order2score);
    ...
    loop {
        ...
        let new_score = state.cal_score(input, &mut order2score);
    ...
    eprintln!("score  : {}", best_state.cal_score(input, &mut order2score));
    best_state
}
...
fn solve(input: &Input, timer:&Instant, tl: f64){
    ...
    // 多点スタート
    for i in 0..2{
        let tl_ = tl*((i+1) as f64/3.0);
        let mut tmp_state = simanneal(input, init_state.clone(), timer, tl_);
        if best_state.cal_score(input, &mut HashMap::new())>tmp_state.cal_score(input, &mut HashMap::new()){
