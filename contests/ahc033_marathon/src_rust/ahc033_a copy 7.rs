    ...
    fn find_ok_pair(&self, op_candidates: Vec<Vec<usize>>, cranes: &Vec<Crane>) -> Vec<usize>{
        ...
        for i in 0..cranes.len(){
            ...
            if let Some(ref task) = *cranes[i].task.borrow(){
                priority[i] += 100;
                if task.is_out_task() && cranes[i].has_item{
                    priority[i] += (100)*(SIZE-task.target_item%SIZE)
                }
            }
            if (cranes[i].p.col==1 || cranes[i].p.col==4 || (cranes[i].p.row==4&&(cranes[i].p.col==2 || cranes[i].p.col==3))){
                priority[i] += 10;
            // } else if cranes[i].has_item {
            //     priority[i] += 20;
            }
        }
...
impl State{
    ...
    fn simulate(&self, input: &Input, conflictsolver: &ConflictSolver) -> (Vec<Crane>, usize, usize){
        ...
        for turn in 0..200{
            // eprintln!("turn:{}", turn);
            // eprintln!("{:?}", task_assigner.task_queue);
            // eprintln!("{:?}", task_assigner.out_task_queue);
            // eprintln!("{:?}", task_assigner.M);
            // for i in 0..cranes.len(){
            //     eprintln!("crane{}in {:?}", i, cranes[i].p);
            //     if let Some(ref task) = *cranes[i].task.borrow(){
            //         eprintln!("task: {:?}", task);
            //     }
            // }
    ...
    fn print(&self, input: &Input, conflictsolver: &ConflictSolver){
        let (cranes, out_cnt, rest_place) = self.simulate(input, conflictsolver);
        // eprintln!("cranes{:?}", cranes);
        for crane in cranes{
        ...
        if let Some(ref task) = *self.task.borrow(){
            if task.status==0{
                ...
                if self.p == task.target_place_p{
                    // ついてたら置く
                    ret = OP_CANDIDATE0_PICK[self.p].clone();
                } else if self.p.col==3 && self.p.row<SIZE-1 && task.is_out_task() && task.previous_out_task.borrow().as_ref().is_some_and(|task_| task_.progress>task.progress){
                    // col3から出る時は先行out_taskの確認をする
                    ret = OP_CANDIDATE0_STAY2[self.p].clone();
                } else if self.p.dist(&task.target_place_p)==1{
                    // ショートカット1: 目的地が隣ならすぐ行く
                    if self.p.row==task.target_place_p.row+1{
                        ret = vec![0, 5];
                    } else if self.p.col+1==task.target_place_p.col{
                        ret = vec![1, 5];
                    } else if self.p.row+1==task.target_place_p.row{
                        ret = vec![2, 5];
                    } else if self.p.col==task.target_place_p.col+1{
                        ret = vec![3, 5];
                    }
                } else if self.p.row>=task.target_place_p.row &&
                        self.p.col<SIZE-1 &&
                        (M[self.p.row][2]==usize::MAX || M[self.p.row][2]==task.target_item) &&
                        (M[self.p.row][3]==usize::MAX || M[self.p.row][3]==task.target_item){
                    // ショートカット2: 右2つが空いているなら行っちゃう
                    ret = vec![1, 5];
                } else if self.p.row==3 && self.p.col==3 && task.target_item<20{
                    // ショートカット3: (3, 3)の時右にいく
                    ret = vec![1, 5];
                } else if self.p.row==100{
                    // ショートカット4: col=2のとき、下が空いていればいく
                    ret = vec![];
                } else if self.p.row==task.target_place_p.row{
                ...
                4 => {
                    if self.has_item{
                        ...
                        if task.is_out_task(){
                            ...
                            task.status = 0;
                            task.is_assigned = false;
                            task.target_place_p = Coordinate::new(usize::MAX, usize::MAX);
                            task.target_item_p = self.p;
                        }
...
impl Task{
    ...
    fn set_progress(&mut self, p: Coordinate){
        if self.target_item<20{
            if self.status == 0{
                self.progress = PROGRESS_MAP2[p] + 1000;
            } else {
                self.progress = PROGRESS_MAP2[p];
            }
        } else {
            if self.status == 0{
    ...
    fn is_ready(&self, M: &Map2d<usize>) -> bool{
        ...
        // space空いている?
        // itemは出現済み?
        if !self.target_place_p.in_map() || (!self.is_out_task() && M[self.target_place_p]!=self.target_item) || M[self.target_item_p] != self.target_item{
            false
...
impl TaskAssigner{
    ...
    fn refresh_tasks(&mut self){
        ...
        // out_task_queueの更新
        for &v in self.item_on_M.iter().sorted(){
                ...
                if !task.is_out_task() && task.is_ready_for_out(){
                    if task.is_assigned{
                        if task.target_place_p.in_map(){
                            ...
                            self.M[task.target_place_p] = usize::MAX;
                        }
        ...
        self.out_task_queue.sort_by_key(|&v| SIZE - v%SIZE);
    }
            ...
            if let Some(ref mut task) = *cranes[i].task.borrow_mut(){
                if !task.target_place_p.in_map(){
                    ...
                    if space_idx==usize::MAX{
                        ...
                        self.M[SPACEIDX2COORDINATE[space_idx]] = task.target_item;
                    } else {
...
fn solve(input: &Input, timer:&Instant, tl: f64){
    ...
    //     }
    // }
    // let mut best_state = simanneal(input, best_state, timer, tl, &conflictsolver);
    // 多点スタート
    for i in 0..10{
        let tl_ = tl*(i as f64/10.0);
        let mut tmp_state = simanneal(input, init_state.clone(), timer, tl_, &conflictsolver);
        if best_state.cal_score(input, &conflictsolver)>tmp_state.cal_score(input, &conflictsolver){
