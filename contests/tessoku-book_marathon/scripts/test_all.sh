...
cp ./src/tessoku-book_a.py ./dev/solve_tmp.py
# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    pypy3 ./dev/solve_tmp.py < "./tools/in/in${f}.txt" > "./tools/out/${f}.txt"
    cd ./tools
    score=$(pypy3 ./judge.py "./in/in${f}.txt" "./out/${f}.txt")
    cd ..
...
if [ $print_error = 0 ]; then
  seq -f '%03g' $st $en | xargs -n1 -P$procs -I{} bash -c "f1 {}"
else
  seq -f '%03g' $st $en | xargs -t -n1 -P$procs -I{} bash -c "f1 {}"
fi
sleep 1
