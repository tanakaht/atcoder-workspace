...
struct GroupTree{
    ...
    roots: HashSet<usize>,
    daily_roots: Vec<HashSet<usize>>,
    startdaywise_roots: Vec<HashSet<usize>>,
}

impl GroupTree{
    fn new(input: &Input) -> Self{
        let mut groups = vec![];
        let mut daily_roots = vec![HashSet::new(); input.D];
        let mut startdaywise_roots = vec![HashSet::new(); input.D];
        for d in 0..input.D{
            for n in 0..input.N{
                groups.push(Group::new(input,  d, d+1, vec![vec![n]]));
                daily_roots[d].insert(d*input.N+n);
                startdaywise_roots[d].insert(d*input.N+n);
            }
        }
        Self{groups, leaves: (0..input.D*input.N).collect(), roots: (0..input.D*input.N).collect(), daily_roots, startdaywise_roots}
    }

    // 0: だめ, 1: join_a, 2: join_t
    fn can_join(&self, idx1: usize, idx2: usize) -> usize{
    ...
    fn random_joint(&mut self, input: &Input){
        ...
        for idx1 in roots{
            for &idx2 in self.roots.iter() {
                ...
                let can_join = self.can_join(idx1, idx2);
                if can_join == 1 {
                    self.join_a(input, idx1, idx2);
                    break;
                } else if can_join == 2 {
                    self.join_t(input, idx1, idx2);
                    break;
                }
            }
        }
    ...
    fn add_root(&mut self, idx: usize){
        ...
        self.startdaywise_roots[self.groups[idx].start_day].insert(idx);
        for d in self.groups[idx].start_day..self.groups[idx].end_day{
            self.daily_roots[d].insert(idx);
        }
    }
    ...
    fn join_a(&mut self, input: &Input, idx1: usize, idx2: usize){
        assert!(self.groups[idx1].start_day==self.groups[idx2].start_day);
        assert!(self.groups[idx1].end_day==self.groups[idx2].end_day);
        let mut new_member_idxs = vec![vec![]; self.groups[idx1].member_idxs.len()];
        for d in 0..self.groups[idx1].member_idxs.len(){
            for &i in self.groups[idx1].member_idxs[d].iter(){
                new_member_idxs[d].push(i);
            }
            for &i in self.groups[idx2].member_idxs[d].iter(){
                new_member_idxs[d].push(i);
            }
        }
        let mut new_group = Group::new(input, self.groups[idx1].start_day, self.groups[idx1].end_day, new_member_idxs);
        let new_group_idx = self.groups.len();
        let max_w = (W*W-std::cmp::min(W*W-100, new_group.space)).sqrt();
        for l in 1..(W+1){
            new_group.line_needed[l] = self.groups[idx1].line_needed[l] + self.groups[idx2].line_needed[l];
        }
        for l in 1..(W+1){
            let l1 = self.groups[idx1].line_needed[l];
            let l2 = self.groups[idx2].line_needed[l];
            if l1+l2<=1000 && new_group.line_needed[l1+l2]>l{
                new_group.line_needed[l1+l2] = l;
            }
        }
        ...
        self.remove_root(idx1);
        self.remove_root(idx2);
        self.add_root(new_group_idx);
    }

    fn join_t(&mut self, input: &Input, idx1: usize, idx2: usize){
        assert!(self.groups[idx1].end_day==self.groups[idx2].start_day);
        assert!(self.groups[idx1].member_idxs[0].len()==self.groups[idx2].member_idxs[0].len());
        let mut new_member_idxs = vec![vec![]; self.groups[idx2].end_day-self.groups[idx1].start_day];
        for d in 0..self.groups[idx1].member_idxs.len(){
            for &i in self.groups[idx1].member_idxs[d].iter(){
                new_member_idxs[d].push(i);
            }
        }
        for d in 0..self.groups[idx2].member_idxs.len(){
            for &i in self.groups[idx2].member_idxs[d].iter(){
                new_member_idxs[d+self.groups[idx1].end_day-self.groups[idx1].start_day].push(i);
            }
        }
        let mut new_group = Group::new(input, self.groups[idx1].start_day, self.groups[idx2].end_day, new_member_idxs);
        let new_group_idx = self.groups.len();
        for l in 1..(W+1){
            new_group.line_needed[l] = std::cmp::max(self.groups[idx1].line_needed[l], self.groups[idx2].line_needed[l]);
        }
        self.groups[idx1].parent_group_idx = Some(new_group_idx);
        self.groups[idx2].parent_group_idx = Some(new_group_idx);
        new_group.child_group_idx.push(idx1);
...
impl Group{
    fn new(input: &Input, start_day: usize, end_day: usize, member_idxs: Vec<Vec<usize>>) -> Self{
        ...
        Self{start_day, end_day, member_idxs, member_sums, space, line_needed, child_group_idx: vec![], parent_group_idx: None, score: i64::MAX}
    }

    fn join_t(input: &Input, gs: &Vec<&Group>) -> Group{
        for i in 0..gs.len()-1{
        ...
        Self{start_day: gs[0].start_day, end_day: gs.last().unwrap().end_day, member_idxs: new_member_idxs, member_sums, space, line_needed, child_group_idx: vec![], parent_group_idx: None, score: i64::MAX}
    }

    fn join_a(input: &Input, g1: &Group, g2: &Group) -> Group{
        ...
        Self{start_day: g1.start_day, end_day: g1.end_day, member_idxs: new_member_idxs, member_sums, space, line_needed, child_group_idx: vec![], parent_group_idx: None, score: i64::MAX}
    }
    ...
    fn solve(&self, input: &Input, rect: (usize, usize, usize, usize), group_tree: &GroupTree) -> HashMap<(usize, usize), (usize, usize, usize, usize)>{
        ...
        if self.member_idxs[0].len() == 1{
            ...
            if group_tree.groups[group1_idx].start_day != group_tree.groups[group2_idx].start_day{
                ans1 = group_tree.groups[group1_idx].solve(input, rect, group_tree);
                ans2 = group_tree.groups[group2_idx].solve(input, rect, group_tree);
            }else if group_tree.groups[group1_idx].line_needed[h]+group_tree.groups[group2_idx].line_needed[h]<=w{
...
fn solve(input: &Input, timer:&Instant, tl: f64) -> HashMap<(usize, usize), (usize, usize, usize, usize)>{
    ...
    for d in 0..input.D{
        while group_tree.daily_roots[d].len()>1{
            ...
            group_tree.join_a(input, *idxs[0], *idxs[1]);
        }
    }
    for d in 1..input.D{
        ...
        group_tree.join_t(input, idx1, idx2);
    }
