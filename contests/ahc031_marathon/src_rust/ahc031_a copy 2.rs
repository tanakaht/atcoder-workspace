...
impl Group{
    ...
    fn divide(&mut self, input: &Input){
        ...
        if best_score<100000000{
            ...
            eprintln!("divided rect: {:?}  w1: {}", self.rect, w1);
        } else {
            ...
            self.child_group = vec![child_group1, child_group2];
            eprintln!("divided duration: {}-{}", self.start_day, self.end_day);
        }
    ...
    fn solve(&self, input: &Input) -> HashMap<(usize, usize), (usize, usize, usize, usize)>{
        ...
        if self.member_idxs[0].len() == 1{
            for d in self.start_day..self.end_day{
                ans.insert((d, self.member_idxs[d][0]), (self.rect.0, self.rect.1, self.rect.2, self.rect.3));
            }
            return ans;
        } else {
