...
problem_name=$1
# cargo build --manifest-path ./Cargo.toml --bin ahc031_a || exit 1
cargo build --manifest-path ./Cargo.toml --bin ahc031_a --release || exit 1

# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    # ./target/debug/ahc031_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt" 2> "./tools/err/${f}.txt"
    ./target/release/ahc031_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt" 2> "./tools/err/${f}.txt"
    cd ./tools
