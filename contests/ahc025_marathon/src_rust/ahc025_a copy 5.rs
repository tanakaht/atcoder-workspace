...
impl Measure{
    fn new() -> Self{
        ...
        loop{
            if (n_small_groups as f64)*(n_small_groups as f64).log2()>((q/2) as f64){
                break
            }
            n_small_groups += d;
        }
        ...
        loop{
            ...
            for (ls, rs, flg) in measured_record{
                ...
                if ls.len()==1 &&rs.len()==1{
                    sample_weight.swap(ls[0], rs[0]);
                } else if rng.gen::<usize>()%2==0{
                    for li in ls.iter(){
                        sample_weight[*li] += (r-l)/ls.len()+10;
                    }
                } else {
                    for ri in rs.iter(){
                        let v  =sample_weight[*ri]-(r-l)/rs.len()-10;
                        if v<sample_weight[*ri]{
                            sample_weight[*ri] = v;
                        }
                    }
                }
    ...
    fn solve(&mut self, timer:&Instant, tl: f64){
        loop{
            ...
            // self.make_group_based_on_estimate(timer, timer.elapsed().as_secs_f64()+0.1);
            // self.print_group(true);

        }
        // let sample_weight = self.get_sample_weight(timer, tl);
        // eprintln!("{:?}", sample_weight);
        // self.make_group_based_on_estimate(timer, tl);
