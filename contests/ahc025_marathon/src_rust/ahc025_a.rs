...
struct Measure{
    ...
    groups_history: Vec<Vec<HashSet<usize>>>,
...
impl Measure{
    fn new() -> Self{
        ...
        // TODO: q/3のところを調整
        while n_small_groups*4<n && (n_small_groups as f64)*(n_small_groups as f64).log2()<(((q as f64)/1.7)){
            n_small_groups += d;
        }
        n_small_groups -= d;
        ...
        let mut ret = Self { measured_record: vec![], groups, groups_history: vec![], small_groups, small_groups_history: vec![], sample_weights: vec![], n, d, q, turn: 0, phase: 0 };
    ...
    fn make_group_based_on_small_group_history2(&mut self, timer:&Instant, tl: f64){
        let (means, vars) = self.estimate(timer, tl);
        let mut best_idx = 0;
        ...
        for (i, small_groups) in self.small_groups_history.iter().enumerate(){
            ...
            for w in group_w.iter(){
                score += (*w as f64-groups_means as f64)*(*w as f64-groups_means as f64);
            }
            if score<best_score{
                best_score = score;
                best_idx = i;
        ...
        let mut small_groups = self.small_groups_history[best_idx].clone();
        small_groups.sort_by_key(|idxs| idxs.iter().map(|i| means[*i]).sum::<usize>());
        let mut q = BinaryHeap::new();
        for i in 0..self.d{
            q.push((Reverse(0), vec![]));
        }
        for idxs in small_groups.iter().rev(){
            let (Reverse(weight_sum), mut group) = q.pop().unwrap();
            group.append(&mut idxs.iter().map(|&i| i).collect_vec());
            q.push((Reverse(weight_sum+idxs.iter().map(|&i| means[i]).sum::<usize>()), group));
        }
        self.groups = vec![];
        for (Reverse(w), g) in q.iter(){
            self.groups.push(g.iter().map(|&i| i).collect());
        }
    }
    ...
    fn solve(&mut self, timer:&Instant, tl: f64){
        ...
        self.make_group_based_on_small_group_history(timer, tl*(self.turn as f64)/(self.q as f64));
        // while self.turn<self.q{
        //     self.move_group(timer, tl*(self.turn as f64)/(self.q as f64));
        //     // self.make_group_based_on_small_group(timer, tl);
        //     self.print_group(true);
        //     // self.estimate(timer, tl*(self.turn as f64)/(self.q as f64));
        // }
        ...
        // self.make_group_based_on_group_history(timer, tl);
        // self.make_group_based_on_small_group(timer, tl);
        // self.make_group_based_on_small_group(timer, tl);
        self.print_group(false);
    }

    fn move_group(&mut self, timer: &Instant, tl: f64){
        let mut rng = rand::thread_rng();
        let (means, vars) = self.estimate(timer, tl);

        let mut ls_idx = *(0..self.d).collect_vec().choose(&mut rng).unwrap();
        let mut rs_idx = *(0..self.d).collect_vec().choose(&mut rng).unwrap();
        while ls_idx==rs_idx{
            ls_idx = *(0..self.d).collect_vec().choose(&mut rng).unwrap();
            rs_idx = *(0..self.d).collect_vec().choose(&mut rng).unwrap();
        }
        let flg = self.measure(&self.groups[ls_idx].iter().map(|&x| x).collect_vec(), &self.groups[rs_idx].iter().map(|&x| x).collect_vec());
        if flg=='>'{
            let tmp = ls_idx;
            ls_idx = rs_idx;
            rs_idx = tmp;
        }
        let mut best_item_idx = *self.groups[rs_idx].iter().choose(&mut rng).unwrap();
        for item_idx in self.groups[rs_idx].iter(){
            if means[best_item_idx]>means[*item_idx]{
                best_item_idx = *item_idx;
            }
        }
        self.groups[ls_idx].insert(best_item_idx);
        self.groups[rs_idx].remove(&best_item_idx);
        let flg2 = self.measure(&self.groups[ls_idx].iter().map(|&x| x).collect_vec(), &self.groups[rs_idx].iter().map(|&x| x).collect_vec());
        if flg2!='<'{
            self.groups[rs_idx].insert(best_item_idx);
            self.groups[ls_idx].remove(&best_item_idx);
        }
    }
    ...
    fn move_small_group(&mut self, timer:&Instant, tl: f64){
        ...
        let mut best_idx = 0;
        let mut best_score = 1000000000000.0;
        let ls_sum = self.small_groups[ls_idx].iter().map(|&i| means[i]).sum::<usize>();
        let rs_sum = self.small_groups[rs_idx].iter().map(|&i| means[i]).sum::<usize>();
        for i in self.small_groups[rs_idx].iter(){
            let score = (((ls_sum+means[*i]) as f64)-((rs_sum-means[*i]) as f64)).abs();
            if score<best_score{
                best_score = score;
                best_idx = *i;
            }
        ...
        self.small_groups[ls_idx].insert(best_idx);
        self.small_groups[rs_idx].remove(&best_idx);
        self.insert_small_group(vec![ls_idx, rs_idx]);
        self.small_groups_history.push(self.small_groups.clone());
        // insert途中で終わってる可能性があるので、元に戻しとく??
        if self.turn==self.q{
            self.small_groups[rs_idx].insert(best_idx);
            self.small_groups[ls_idx].remove(&best_idx);
        }
    }


    fn move_small_group2(&mut self, timer:&Instant, tl: f64){
        let mut rng = rand::thread_rng();
        let sorted_idxs = self.toporogical_sort_for_small_group();
        let mut ls_idx = *sorted_idxs.first().unwrap();
        let mut ri = 1;
