...
problem_name=$1
# cargo build --manifest-path ./Cargo.toml --bin ahc025_a || exit 1
...
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    # ./target/debug/ahc025_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    # ./target/release/ahc025_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    cd ./tools
    cargo run -r --bin tester ../target/release/ahc025_a  < "./in/${f}.txt" > "./out/${f}.txt" 2> "./err/${f}.txt"
