...
problem_name=a
cargo build --manifest-path ./Cargo.toml --bin --release rco-contest-2019-qual_{problem_name} || exit 1

# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    ./target/debug/rco-contest-2019-qual_${problem_name}  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    cd ./rco-contest-2019
    score=$(python3 ./qual_${problem_name}/tester_py/judge.py input.txt output.txt 2>/dev/null | awk '{ print $3 }')
    cd ..
