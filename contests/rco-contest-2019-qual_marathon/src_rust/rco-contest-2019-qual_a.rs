...
impl State {
    fn init(input: &Input, seed: u128) -> Self {
        ...
        let mut daily_dijkstra: Vec<Vec<Dijkstra>> = vec![vec![]; input.D];
        let mut daily_score: Vec<usize> = vec![0; input.D];
        for u in sample_nodes.iter(){
            let dijkstra = Dijkstra::new(&input.G, *u, None, &HashSet::new());
            for d in 0..input.D{
                daily_dijkstra[d].push(dijkstra.clone());
            }
        }
        // 初期アサイン
        for (i, (u, v, w)) in input.UVW.iter().enumerate(){
            let mut best_d: usize = 0;
            let mut best_score_diff: i64 = INF as i64;
            for d in 0..input.D{
                if assigns[d].len()>=input.K{
                    continue;
                }
                let mut score_diff = 0;
                for dijkstra in daily_dijkstra[d].iter_mut(){
                    score_diff -= dijkstra.total_dist as i64;
                    assigns[d].insert(i);
                    dijkstra.recalculate(input, &assigns[d], &vec![], &vec![i]);
                    score_diff += dijkstra.total_dist as i64;
                    assigns[d].remove(&i);
                    dijkstra.recalculate(input, &assigns[d], &vec![i], &vec![]);
                }
            ...
            assigns[best_d].insert(i);
            for dijkstra in daily_dijkstra[best_d].iter_mut(){
                dijkstra.recalculate(input, &assigns[best_d], &vec![], &vec![i]);
            }
        }
        for d in 0..input.D{
            for dijkstra in daily_dijkstra[d].iter(){
