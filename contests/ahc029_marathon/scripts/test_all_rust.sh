...
problem_name=$1
# cargo build --manifest-path ./Cargo.toml --bin ahc029_a || exit 1
...
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    # ./target/debug/ahc029_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    ./tools/target/release/tester ./target/release/ahc029_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt" 2> "./tools/err/${f}.txt"
    cd ./tools
    score=$(./target/release/vis "./in/${f}.txt" "./out/${f}.txt" |& awk '{ print $3 }')
    cd ..
