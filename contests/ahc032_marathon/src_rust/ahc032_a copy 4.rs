...
impl State{
    fn init_state(input: &Input) -> Self{
        ...
        for i in 0..input.n-3{
            for j in 0..input.n-3{
                let c = Coordinate::new(i, j);
                ...
                if best_score>ret.M[c]%MOD{
                    ret.update(input, &Neighbor{params: vec![(0, best_m, c)]});
                }
            }
            let c1 = Coordinate::new(i, input.n-3);
            let c2 = Coordinate::new(i, input.n-2);
            let c3 = Coordinate::new(i, input.n-1);
            let mut best_score = 0;
            let mut best_m = vec![];
            for m1 in 0..input.m{
                let mut score = 0;
                score += (input.S[m1][0]+ret.M[c1])%MOD;
                score += (input.S[m1][1]+ret.M[c2])%MOD;
                score += (input.S[m1][2]+ret.M[c3])%MOD;
                if score>best_score{
                    best_score = score;
                    best_m = vec![m1];
                }
                for m2 in 0..input.m{
                    let mut score = 0;
                    score += (input.S[m1][0]+input.S[m2][0]+ret.M[c1])%MOD;
                    score += (input.S[m1][1]+input.S[m2][1]+ret.M[c2])%MOD;
                    score += (input.S[m1][2]+input.S[m2][2]+ret.M[c3])%MOD;
                    if score>best_score{
                        best_score = score;
                        best_m = vec![m1, m2];
                    }
                for m3 in 0..input.m{
                        let mut score = 0;
                        score += (input.S[m1][0]+input.S[m2][0]+input.S[m3][0]+ret.M[c1])%MOD;
                        score += (input.S[m1][1]+input.S[m2][1]+input.S[m3][1]+ret.M[c2])%MOD;
                        score += (input.S[m1][2]+input.S[m2][2]+input.S[m3][2]+ret.M[c3])%MOD;
                        if score>best_score{
                            best_score = score;
                            best_m = vec![m1, m2, m3];
                        }
            ...
            if best_score>ret.M[c1]%MOD+ret.M[c2]%MOD+ret.M[c3]%MOD{
                let params = best_m.iter().map(|&m| (0, m, c1)).collect();
                ret.update(input, &Neighbor{params});
            }
        }
        for j in 0..input.n-3{
            let c1 = Coordinate::new(input.n-3, j);
            let c2 = Coordinate::new(input.n-2, j);
            let c3 = Coordinate::new(input.n-1, j);
            let mut best_score = 0;
            let mut best_m = vec![];
            for m1 in 0..input.m{
                let mut score = 0;
                score += (input.S[m1][0]+ret.M[c1])%MOD;
                score += (input.S[m1][3]+ret.M[c2])%MOD;
                score += (input.S[m1][6]+ret.M[c3])%MOD;
                if score>best_score{
                    best_score = score;
                    best_m = vec![m1];
                }
                for m2 in 0..input.m{
                    let mut score = 0;
                    score += (input.S[m1][0]+input.S[m2][0]+ret.M[c1])%MOD;
                    score += (input.S[m1][3]+input.S[m2][3]+ret.M[c2])%MOD;
                    score += (input.S[m1][6]+input.S[m2][6]+ret.M[c3])%MOD;
                    if score>best_score{
                        best_score = score;
                        best_m = vec![m1, m2];
                    }
                for m3 in 0..input.m{
                        let mut score = 0;
                        score += (input.S[m1][0]+input.S[m2][0]+input.S[m3][0]+ret.M[c1])%MOD;
                        score += (input.S[m1][3]+input.S[m2][3]+input.S[m3][3]+ret.M[c2])%MOD;
                        score += (input.S[m1][6]+input.S[m2][6]+input.S[m3][6]+ret.M[c3])%MOD;
                        if score>best_score{
                            best_score = score;
                            best_m = vec![m1, m2, m3];
                        }
        ...
        for m1 in 0..input.m{
            ...
            for m2 in 0..input.m{
                ms.push(vec![m1, m2]);
                for m3 in 0..input.m{
                    ms.push(vec![m1, m2, m3]);
                }
            }
        }
        for m_ in ms.iter(){
            let mut stump_sum = vec![0; 9];
            for (i, c) in cs.iter().enumerate(){
                stump_sum[i] += input.A[c];
            }
            for m in m_.iter(){
                for (i, &d) in input.S[*m].iter().enumerate(){
                    stump_sum[i] = (stump_sum[i]+d)%MOD;
                }
            }
            let score = stump_sum.iter().sum::<usize>();
            if score>best_score{
                best_score = score;
                best_m = m_.clone();
            }
        }
        let params = best_m.iter().map(|&m| (0, m, cs[0])).collect();
        ret.update(input, &Neighbor{params});
        ret
    }
    ...
    fn print(&mut self, input: &Input){
        ...
        for ((m, p), cnt) in self.cnts.iter().sorted_by_key(|(k, v)|k.1.col*input.n+k.1.row){
            for _ in 0..*cnt{
...
fn solve(input: &Input, timer:&Instant, tl: f64){
    ...
    let mut best_state = simanneal(input, init_state, timer, tl*1.0/4.0);
