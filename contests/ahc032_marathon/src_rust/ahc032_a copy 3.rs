...
impl State{
    ...
    fn init_state2(input: &Input) -> Self{
        ...
        for i in (0..input.n-2).rev(){
            for j in (0..input.n-2).rev(){
                let c = Coordinate::new(i, j);
                let c_ = Coordinate::new(i+2, j+2);
                let mut best_score = 0;
                let mut best_m = 0;
                for m in 0..input.m{
                    let score = (input.S[m][8]+ret.M[c_])%MOD;
                    if score>best_score{
                        best_score = score;
                        best_m = m;
                    }
    ...
    fn init_state3(input: &Input) -> Self{
        let mut score = 0;
        let mut score_weighted = 0;
        for i in 0..input.n{
            for j in 0..input.n{
        ...
        for i in (0..input.n-2).rev(){
            for j in (0..input.n-2){
                let c = Coordinate::new(i, j);
                let c_ = Coordinate::new(i+2, j);
                let mut best_score = 0;
                let mut best_m = 0;
                for m in 0..input.m{
                    let score = (input.S[m][6]+ret.M[c_])%MOD;
                    if score>best_score{
                        best_score = score;
                        best_m = m;
                    }
    ...
    fn init_state4(input: &Input) -> Self{
        let mut score = 0;
        let mut score_weighted = 0;
        for i in 0..input.n{
            for j in 0..input.n{
        ...
        for i in (0..input.n-2){
            for j in (0..input.n-2).rev(){
                let c = Coordinate::new(i, j);
                let c_ = Coordinate::new(i, j+2);
                let mut best_score = 0;
                let mut best_m = 0;
                for m in 0..input.m{
                    let score = (input.S[m][2]+ret.M[c_])%MOD;
                    if score>best_score{
                        best_score = score;
                        best_m = m;
                    }
...
fn solve(input: &Input, timer:&Instant, tl: f64){
    let mut init_state1 = State::init_state(input);
    let mut best_state1 = simanneal(input, init_state1, timer, tl*1.0/4.0);
    let mut init_state2 = State::init_state2(input);
    let mut best_state2 = simanneal(input, init_state2, timer, tl*2.0/4.0);
    let mut init_state3 = State::init_state3(input);
    let mut best_state3 = simanneal(input, init_state3, timer, tl*3.0/4.0);
    let mut init_state4 = State::init_state4(input);
    let mut best_state4 = simanneal(input, init_state4, timer, tl*4.0/4.0);
    let best_states = vec![best_state1, best_state2, best_state3, best_state4];
    let mut best_score = 0;
    let mut best_state = best_states[0].clone();
    for state in best_states.iter(){
        let score = state.get_score(input).0;
        if score>best_score{
            best_score = score;
            best_state = state.clone();
        }
