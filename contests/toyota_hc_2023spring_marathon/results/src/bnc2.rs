...
impl Field{
    ...
    fn get_mid_score(&self) -> usize{
        ...
        let mut score = 1000+inv_cnt*1000+{if (max_height>self.D){10000000} else {0}};
        score
    }
    ...
    fn get_raw_score(&self) -> usize{
        ...
        let order_idx: Vec<usize> = order.iter().map(|idx| self.placedloads[*idx].load.idx).collect();
        let inv_cnt = inversion(&order_idx);
        let mut score = 1000+(max_height as usize)+inv_cnt*1000+{if (max_height>self.D){10000000} else {0}};
        score
    }
...
fn beam_search(input: &Input, order: &Vec<usize>, beam_width: usize) -> Option<Field>{
...
fn chokudai_search(input: &Input, timer: &Instant, tl: f64, beam_width: usize) -> Field{
    ...
    if heaps[input.loads.len()].is_empty(){
        let mut ret = Field::new(input);
        ret.score = Some(INF);
        ret
    } else{
...
impl State{
    fn get_neighbor(&self, input: &Input) -> State{
        let mut ret = self.clone();
        let mut rng = rand::thread_rng();
        while true{
            let i = rng.gen::<usize>()%ret.order.len();
            let j = rng.gen::<usize>()%ret.order.len();
            let (oi, oj) = (ret.order[i], ret.order[j]);
            if input.loads[oi].idx!=input.loads[oj].idx{
                ret.order[i] = oj;
                ret.order[j] = oi;
                break;
            }
    ...
    fn get_score(&mut self, input: &Input) -> usize{
        if let Some(score) = self.score{
            return score
        }
        let best_field = beam_search(input, &self.order, 10);
        if let Some(field) = &best_field{
            self.score = field.score;
        } else {
            self.score = Some(INF);
        }
...
impl Display for State {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(field)=&self.best_field {
            write!(f, "{}", field)?;
            Ok(())
        } else {
            write!(f, "")?;
...
fn simanneal(input: &Input, init_state: State, timer: &Instant, limit: f64) -> State{
    ...
    loop {
        ...
        all_iter += 1;
        let mut neighbor = state.get_neighbor(input);
        let score_diff:i128 = (neighbor.get_score(input) as i128)-(state.get_score(input) as i128);
        temp = start_temp + (end_temp-start_temp)*elasped_time/limit;
        if (score_diff<=0 || ((-score_diff as f64)/temp).exp() > rng.gen::<f64>()){
            state = neighbor;
            accepted_cnt += 1;
            eprintln!("{}", state.get_score(input));
...
fn solve(input: &Input, timer:&Instant){
    let mut best_state = solve_simanneal(input, timer, 0.7);
    let mut best_field = solve_chokudai(input, timer, 1.4);
    eprint!("beam: {}", best_state.get_score(input));
    eprint!("chokudai: {}", best_field.get_score());
    if best_state.get_score(input)>best_field.get_score(){
        println!("{}", best_field);
    } else {
        println!("{}", best_state);
    }
...
fn solve_chokudai(input: &Input, timer:&Instant, tl: f64) -> Field{
    let best_fields = chokudai_search(input, &timer, tl, 1);
    //println!("{}", best_fields);
    eprintln!("{}", timer.elapsed().as_secs_f64());
    best_fields
}

fn solve_simanneal(input: &Input, timer:&Instant, tl: f64) -> State{
    let mut order = vec![];
    ...
    let init_state = State{order, best_field: None, score: Some(INF)};
