...
impl Field{
    ...
    fn get_score(&self, input: &Input) -> usize{
        ...
        let mut score = 1000+inv_cnt*1000+{if (max_height>input.D){100000000000} else {0}};
        score
    }

    fn get_raw_score(&self, input: &Input) -> usize{
        let mut max_height = 0;
        for pl in self.placedloads.iter(){
            if pl.is_dummy{
                continue;
        ...
        let order_idx: Vec<usize> = order.iter().map(|idx| self.placedloads[*idx].load.idx).collect();
        let inv_cnt = inversion(&order_idx);
        let mut score = 1000+(max_height as usize)+inv_cnt*1000+{if (max_height>input.D){100000000000} else {0}};
        score
    }
...
fn beam_search(input: &Input, order: &Vec<usize>, beam_width: usize) -> Vec<Field>{
    ...
    for (i, idx) in order.iter().enumerate(){
        ...
        if i+1==order.len(){
            fields_new.sort_by_cached_key(|x| x.get_raw_score(input));
...
fn solve(input: &Input, timer:&Instant){
    ...
    let fields = beam_search(input, &order, 100);
    if !fields.is_empty(){
        println!("{}", fields[0]);
    }
