...
impl Block{
    ...
    fn rotate(&self, xr: usize, yr: usize) -> Block{
        let mut ret = self.clone();
        let mut X = rotate_x(&ret.is_filled, xr);
        X = rotate_y(&X, yr);
        ret.is_filled = X.to_owned();
        ret
    }
...
impl Field{
    ...
    fn get_new_block_ids(&self, n: usize) -> Vec<usize>{
        let mut ret = vec![];
        for id in 1..50000{
            if !self.blocks.contains_key(&id){
                ret.push(id)
...
impl State{
    ...
    fn get_neighbor_split_block(&mut self) -> (Vec<(usize, (usize, usize, usize))>, Vec<(usize, (usize, usize, usize))>){
        let mut rng = rand::thread_rng();
        let (block_id1, block1) = self.field1.blocks.iter().choose(&mut rng).unwrap();
        for (block_id2, block2) in self.field2.blocks.iter(){
            if block1.serial!=block2.serial{
                continue;
            }
            let mut change_block_info_1 = vec![];
            let mut change_block_info_2 = vec![];
            let cubes1 = block1.get_cubes();
            let cubes2 = block2.get_cubes();
            for (xyz, new_block_id) in cubes1.iter().zip(self.field1.get_new_block_ids(cubes1.len())){
                change_block_info_1.push((new_block_id, *xyz));
            }
            for (xyz, new_block_id) in cubes2.iter().zip(self.field2.get_new_block_ids(cubes2.len())){
                change_block_info_2.push((new_block_id, *xyz));
            }
    ...
    fn get_neighbor(&mut self) -> (Vec<(usize, (usize, usize, usize))>, Vec<(usize, (usize, usize, usize))>){
        ...
        if mode_flg < 30{
            ...
            return self.get_neighbor_split_block();
        } else {
            // 近傍4: ブロックを移動
    ...
    fn get_my_score(&mut self) -> i64{
        ...
        for block in self.field1.blocks.values(){
            if block.n_cube>0{
                score += 1000/(block.n_cube as i64);
    ...
    fn get_my_score2(&mut self) -> i64{
        ...
        let mut single_cube_cnt = 10000000;
        if let Some(adj) = self.field1.adj.get(&self.field1.none_block_serial){
            if let Some(adj_) = adj.get(&self.field1.none_block_serial){
                let mut cubes = HashSet::new();
                for (xyz, xyz_) in adj_.iter(){
                    cubes.insert(*xyz);
                }
                single_cube_cnt = min!(single_cube_cnt, cubes.len()-1);
            }
        }
        if let Some(adj) = self.field2.adj.get(&self.field2.none_block_serial){
            if let Some(adj_) = adj.get(&self.field2.none_block_serial){
                let mut cubes = HashSet::new();
                for (xyz, xyz_) in adj_.iter(){
                    cubes.insert(*xyz);
                }
        ...
        if single_cube_cnt < 10000000{
            score -= (single_cube_cnt*100000) as i64;
        }
        score
    }
...
fn simanneal(input: &Input, init_state: State, timer: &Instant, limit: f64) -> State{
    ...
    loop {
        ...
        all_iter += 1;
        // if all_iter-last_updated>5000{
        //     eprintln!("split");
        //     let mut neighbor = state.get_neighbor_split_block();
        ...
        // }
        // let mode: usize = 0;
        let mut mode: usize = if (elasped_time <= limit*3.0/4.0) {0} else {1};
        let pre_score = state.get_score(mode);
        let mut neighbor = state.get_neighbor();
        if rng.gen::<usize>()%100<3{
            neighbor = state.get_neighbor();
            mode = 2;
        }
        let is_success = state.update_if_ok(&neighbor.0, &neighbor.1);
        if !is_success{
    ...
    eprintln!("score  : {}", best_state.get_raw_score());
    eprintln!("");
    best_state
}
