...
impl State{
    fn init_state(input: &Input, timer: &Instant, tl: f64) -> Self{
        ...
        let mut ids = vec![];
        for _ in 0..input.n{
            ids.push(rng.gen::<usize>()%(1<<7));
        }
        ids = Self::get_ids(input, &id_points, &Self::get_key_points(input, &id_points, &ids));
        let mut same_points = vec![];

        ids = get_unoverlap_n_ids(n: usize, m:usize, same_points: &Vec<((usize, usize), (usize, usize))>, timer: &Instant, tl: f64);
...
fn main() {
    ...
    // get arrangement
    let arrangement = Arrangement::new(&input, &timer, 320.0);
