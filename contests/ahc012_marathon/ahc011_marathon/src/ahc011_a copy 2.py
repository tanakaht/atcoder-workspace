...
class UnionFindDisjoint:
    ...
    def rooten(self, x):
        pre = x
        cur = self.parents[pre]
        self.parents[x] = -1
        while cur>=0:
            next = self.parents[cur]
            self.parents[cur] = pre
            pre = cur
            cur = next
...
while time()-TS<270:
    ...
    temp = start_temp
    while temp>=end_temp and time()-TS<270:
        cnt += 1
        temp = temp*temp_c
        print(cnt, temp, score)
        # 最終盤面の探索
        ...
        v1 = v1_pre^(1<<dir)
        v2 = v2_pre^(1<<((dir+2)%4))
        score = sum([abs(v_cnt_S[i]-v_cnt_S_ideal[i]) for i in range(16)])
        v_cnt_S_ideal[v1_pre] -= 1
        v_cnt_S_ideal[v2_pre] -= 1
        v_cnt_S_ideal[v1] += 1
        v_cnt_S_ideal[v2] += 1
        score_ = sum([abs(v_cnt_S[i]-v_cnt_S_ideal[i]) for i in range(16)])
        if random.random() < math.exp((score-score_)/temp) and (v1_pre>v1 or uf.find(p1[0]*N+p1[1])!=uf.find(p2[0]*N+p2[1])):
