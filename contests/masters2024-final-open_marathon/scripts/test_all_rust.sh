...
# cargo build --manifest-path ./Cargo.toml --bin masters2024-final-open_a --release || exit 1

# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    ./target/debug/masters2024-final-open_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    # ./target/release/masters2024-final-open_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    cd ./tools
