...
problem_name=$1
# cargo build --manifest-path ./Cargo.toml --bin ahc023_a || exit 1
cargo build --manifest-path ./Cargo.toml --bin ahc023_a --release || exit 1

# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    # ./target/debug/ahc023_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    ./target/release/ahc023_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt" 2> "./tools/err/${f}.txt"
    cd ./tools
