...
impl State{
    ...
    fn get_reachmap_with_invalid_with_target(&self, input: &Input, invalid: usize, from: (Flg, usize)) -> (Vec<Flg>, Vec<Flg>){
        ...
        // TODO:
        for art_point in input.art_points.iter(){
            if !input.art_point_children[*art_point].contains(&invalid){
                for idx in input.art_point_children[*art_point].iter(){
                    if idx!=art_point{
                        flgs_with_valid[*idx] = Flg::zero();
                        flgs[*idx] = Flg::zero();
                    }
                }
            }
    ...
    fn optimize_art_point(&mut self, input: &Input, subtree_root: usize, parents: &Vec<Option<usize>>, childrens: &Vec<Vec<usize>>){
        assert!(input.art_points.contains(&subtree_root));
        ...
        let mut flg_subtree_root = Flg::zero();
        ...
        for &idx in input.get_ancestors(parents, subtree_root).iter(){
            ...
            if idx==subtree_root{
                flg_subtree_root = flgs[subtree_root].clone();
            }
        ...
        for idx in input.get_subtree_idxs(childrens, subtree_root).iter().rev(){
            ...
            // neighbor取得してupdate
            let neighborparam = self.get_neighbor_optimize_single_place_with_target(input, *idx, &costs, (flg_subtree_root.clone(), subtree_root));
    ...
    fn get_should_open_with_target(&self, input: &Input, idx: usize, flgs_with_invalid: Vec<Flg>, flgs: Vec<Flg>) -> Flg{
        ...
        for (i, (flg_with_invalid, flg)) in flgs_with_invalid.iter().zip(flgs.iter()).enumerate(){
            ...
            let mut idxs = self.place2idx[i].iter().map(|&c| c).collect_vec();
            if input.art_points.contains(&idx) && !input.art_point_children[idx].contains(&i){
                idxs = vec![];
                for i_ in input.art_point_children[idx].iter(){
...
fn simanneal(input: &Input, init_state: State, timer: &Instant, limit: f64) -> State{
    ...
    loop {
        ...
        while (!input.art_points.contains(&idx1)) || (input.get_subtree_idxs(&childrens, idx1).len()<5 && input.get_subtree_idxs(&childrens, idx1).len()>=400){
...
fn solve(input: &Input, timer:&Instant, tl: f64){
    ...
    eprintln!("{:?}", input.art_point_children);
