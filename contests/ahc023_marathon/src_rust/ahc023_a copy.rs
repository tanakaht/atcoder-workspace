...
impl State{
    fn init_state(input: &Input) -> Self{
        ...
        let mut state = Self{places: vec![input.h*input.w; input.k], place2idx, reachmap: None};
        // moveを作る
        // dfsord
        let mut dfs_ord = vec![];
        let mut q = vec![];
        ...
        let neighborparam = NeighborParam{moves, reachmap: None};
    ...
    fn update_reachmap(&mut self, input: &Input){
        if self.reachmap.is_some(){
            return;
        }
        let mut flgs = vec![Flg::zero(); input.h*input.w];  // 入った時に更新
        let mut last_updated = vec![Flg::zero(); input.h*input.w];  // q入れた時に更新
        let mut q = VecDeque::new();
        q.push_back((input.i0*input.w, self.get_mapflg(input, input.i0*input.w)));
        while !q.is_empty(){
            let (idx, flg) = q.pop_front().unwrap();
            ...
            for idx2 in input.g[idx].iter(){
                ...
                if !flg2.eq(&last_updated[*idx2]) && last_updated[*idx2].is_bigger(&flg2){
                    ...
                    q.push_back((*idx2, flg2));
                }
            }
        }
        self.reachmap = Some(flgs);
    }


    fn is_ok(&mut self, input: &Input) -> bool{
        ...
        self.update_reachmap(input);
        for (i, flg_) in self.reachmap.as_ref().unwrap().iter().enumerate(){
    ...
    // i番目のアイテムがidxの場所に入れそうか
    fn may_join(&mut self, i: usize, idx: usize, input: &Input) -> bool{
        if idx==input.h*input.w{
            return true;
        }
        self.update_reachmap(input);
        let flg = self.reachmap.as_ref().unwrap()[idx].clone();
        let (s, d) = input.sd[i];
        if !flg.is_ok(d){
            return false;
        }
        if let Some(s_) = flg.max_bit(s){
            for j in self.place2idx[idx].iter(){
                let (s2, d2) = input.sd[*j];
                if (min!(d, d2)<max!(s_, s2)){
                    return false;
                }
            }
            true
    ...
    fn get_neighbor(&mut self, input: &Input) -> NeighborParam{
        ...
        if mode_flg<80{
            ...
            for _ in 0..1000 {
                ...
                let k = rng.gen::<usize>()%(input.h*input.w);
                // そこに入れられるか
                if !(k==self.places[i]) && self.may_join(i, k, input){
                    moves.push((i, self.places[i], k));
                    break;
                }
            }
            ...
            for _ in 0..1000 {
                ...
                // そこに入れられるか
                if !(k1==k2) && self.may_join(i1, k2, input) && self.may_join(i2, k1, input){
                    moves.push((i1, k1, k2));
                    moves.push((i2, k2, k1));
                    break;
                }
        ...
        NeighborParam{moves, reachmap: self.reachmap.clone()}
    }
        ...
        if self.is_ok(input){
            ...
            for (i, place) in self.places.iter().enumerate(){
                if *place<input.h*input.w{
                    score += input.sd[i].1-input.sd[i].0+1;
                }
            }
    ...
    fn print(&mut self, input: &Input){
        ...
        let mut anss = vec![];
        self.update_reachmap(input);
        for (i, flg) in self.reachmap.as_ref().unwrap().iter().enumerate(){
...
fn simanneal(input: &Input, init_state: State, timer: &Instant, limit: f64) -> State{
    ...
    loop {
        ...
        if score_diff>=0.0 || rng.gen_bool((score_diff as f64/temp).exp()){
            eprintln!("{} {:?} {}", new_score, neighbor.moves, all_iter);
            accepted_cnt += 1;
