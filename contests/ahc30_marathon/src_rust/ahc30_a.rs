...
const COLORPALLET: [&str; 24] = ["#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#1007f0", "#10010033", "#a65628", "#f781bf", "#999999", "#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#1007f0", "#10010033", "#a65628", "#f781bf", "#999999", "#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#1007f0", "#10010033"];
...
struct Problem{
    ...
    all_select_cnt: usize,
}

impl Problem{
    fn new(input: &Input) -> Self{
        let mut ret = Self{nodes: vec![], rows: vec![], head_nodes: vec![], tail_nodes: vec![], rowhead_nodes: vec![], rowtail_nodes: vec![], unsatisfied_columns: HashSet::new(), columns_satisfied_cnt: vec![], columns_element_cnt: vec![], rest_columns: 0, row2minoidx_coordinates: vec![], covered_row_cnt: 0, all_select_cnt: 0};
        for _ in 0..input.m{
    ...
    fn get_mino_coords(&mut self, input: &Input, n_min_ans: usize, timer:&Instant, tl: f64)->Vec<Vec<Coordinate>>{
        let mut rng = rand::thread_rng();
        let mut ret = vec![];
        let bef_cnt = self.all_select_cnt;
        let n_max_ans = 1000;
        eprintln!("start_solve_problem {}*{}", self.rows.len()-self.covered_row_cnt, self.head_nodes.len());
        for v in self.solve_problem(n_min_ans, timer, tl, 1, 0).0{
            let mut candidates: Vec<Vec<(usize, Coordinate)>> = vec![vec![]];
            for i in v.iter(){
                let mut new_candidates = vec![];
                let mut minoidx_c = self.row2minoidx_coordinates[*i].to_vec();
                minoidx_c.shuffle(&mut rng);
                for (mino_idx, c) in minoidx_c.iter(){
                    for candidate in candidates.iter(){
                        let mut new_candidate = candidate.clone();
                        new_candidate.push((*mino_idx, *c));
                        new_candidates.push(new_candidate);
                        if new_candidates.len()>=n_max_ans*2{
                            break;
                        }
                    }
                    if new_candidates.len()>=n_max_ans*2{
                ...
                candidates = new_candidates.choose_multiple(&mut rng, n_max_ans).map(|f|f.to_vec()).collect();
            }
            for candidate in candidates.iter(){
        ...
        // eprintln!("select_cnt: {}, {}", self.all_select_cnt-bef_cnt, self.all_select_cnt);
        ret
    }
}
...
impl Candidate{
    ...
    fn get_score_1(&mut self, input: &Input, record: &Record) -> f64{
        ...
        for c in record.ng_coords.iter(){
            score_1 += (self.V[*c]*100) as f64;
        }
    ...
    fn eprint(&self, input: &Input, v: &Vec<(usize, Coordinate)>){
        for (i, c) in self.ans_.iter(){
            for c_ in input.minos[*i].get_blocks(*c){
                println!("#c {} {} black", c_.row, c_.col);
            }
        }
        for (i, c) in v.iter(){
            for c_ in input.minos[*i].get_blocks(*c){
                println!("#c {} {} red", c_.row, c_.col);
            }
        }
...
impl Solver{
    fn new(input: &Input, n_candidates: usize) -> Self{
        ...
        Self {record, turn:0, n_candidates, measured: HashSet::new(), total_iter: 0, problem: Problem::new(input), candidates: vec![]}
    }

    fn solve3(&mut self, input: &Input){
        eprintln!("solve3");
        let mut rng = rand::thread_rng();
        let mut candidate = Candidate::from_ans(input, vec![None; input.m]);
        loop{
            if self.record.oil_cnts.values().sum::<usize>()==input.oil_cnt{
                let mut oil_coords = vec![];
                for (c, v) in self.record.oil_cnts.iter(){
                    if *v>0{
                        oil_coords.push(*c);
                    }
                }
            ...
            let mut flg = true;
            let mut ng_coords = self.record.ng_coords.clone();
            while flg{
                flg = false;
                for (c, v) in self.record.oil_cnts.iter(){
                    if *v==candidate.V[*c]{
                ...
                for (mino_idx, mino) in input.minos.iter().enumerate(){
                    if candidate.ans[mino_idx].is_some(){
                        continue;
                    }
                    if mino.get_oks(&ng_coords).len()==1{
                        candidate.update(input, &Neighbor{moves: vec![(mino_idx, Some(*mino.get_oks(&ng_coords).iter().next().unwrap()))]});
                        flg = true;
                        break;
                    }
                }
            ...
            for (mino_idx, mino) in input.minos.iter().enumerate(){
                ...
                for c in mino.get_oks(&ng_coords){
                    for c_ in mino.get_blocks(c){
                        if !self.measured.contains(&c_){
                            candidate.eprint(input, &vec![(mino_idx, c)]);
                            self.measure(input, 0, vec![c_]);
                            for i in 0..input.n{
                                for j in 0..input.n{
                                    println!("#c {} {} white", i, j);
                                }
    ...
    fn solve(&mut self, input: &Input, timer: &Instant, tl_: f64){
        // let mut v = vec![];
        // for x in 0..input.n*input.n{
        //     let lblock = x/(input.n.sqrt()*input.n);
        //     let haba = ((lblock+1)*input.n.sqrt()).min(input.n)-lblock*input.n.sqrt();
        //     let rest = x%(input.n.sqrt()*input.n);
        //     v.push(Coordinate::new(lblock*input.n.sqrt()+rest%haba, rest/haba));
        //     if v.len()==input.n{
        //         self.measure(input, 1, v);
        //         v = vec![];
        //     }
        // }
        ...
        while self.turn<max_turn{
            ...
            if self.candidates.len()<2{
                for ans in self.problem.get_mino_coords(input, self.n_candidates, timer, (tl*(self.turn+1) as f64)/max_turn as f64){
                    ...
                    self.candidates.push(Candidate::from_ans(input, v));
                }
            }
            if !self.record.a.is_empty(){
                let candidates = self.candidates.to_owned();
                self.candidates = vec![];
                for candidate in candidates{
                    if !candidate.is_answered(input, &self.record){
            ...
            if self.candidates.len()<4{
                for ans in self.problem.get_mino_coords(input, self.n_candidates, timer, tl){
                    let v = ans.iter().map(|c| Some(*c)).collect_vec();
                    let candidate = Candidate::from_ans(input, v);
                    let mut same = false;
                    for candidate_ in self.candidates.iter(){
            ...
            for i in 0..input.n{
                for j in 0..input.n{
                    ...
                    for candidate in self.candidates.iter().take(if self.record.qd.is_empty() {self.candidates.len()} else {(self.n_candidates*2/3).min(self.candidates.len())}){
            ...
            // let mut rng = rand::thread_rng();
            // // for (i, cand) in self.candidates.iter().choose_multiple(&mut rng, 10).iter().enumerate(){
            // for (i, cand) in self.candidates.iter().take(self.n_candidates).enumerate(){
            //     for c in cand.get_oil_coords(){
            //         println!("#c {} {} {}", c.row, c.col, COLORPALLET[i]);
            //     }
            // }
    ...
    fn filter_candidate(&mut self, input: &Input){
        self.candidates = vec![];
        return;
        // TODO: 保持しない方が結果が良いので保持しない
        for cand in self.candidates.iter_mut(){
            cand.get_score_1(input, &self.record);
        }
