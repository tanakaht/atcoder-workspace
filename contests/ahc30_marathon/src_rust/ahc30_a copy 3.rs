...
impl Input {
    fn read_input() -> Self {
        ...
        for _ in 0..m {
            ...
            centers.push(CoordinateDiff::new(rd/d, cd/d));
        }
        Self { n, m, e, D, max_cs, oil_cnt, centers }
    }
}
...
impl Candidate{
    ...
    fn get_score_1(&mut self, input: &Input, record: &Record) -> f64{
        ...
        let mut score_1 = 0.0;
        for i in 0..input.m{
            if !record.valid[i].contains(&self.ans[i]){
                score_1 += 200.0;
            }
        }
        for (c, v) in record.oil_cnts.iter(){
            ...
            score_1 += (self.V[*c].abs_diff(*v)*100) as f64;
            // 足りないなら最短距離を、多いなら最短距離の逆数を加算
            if self.V[*c]<*v{
                let mut mindist = usize::MAX;
                for i in 0..input.m{
                    let mut mindist_c = usize::MAX;
                    let mut is_on = false;
                    for cd in input.D[i].iter(){
                        let c_ = self.ans[i]+*cd;
                        if c_==*c{
                            is_on = true;
                            break;
                        }
                        mindist_c = mindist_c.min(c_.dist(c));
                    }
                    if !is_on{
                        mindist = mindist.min(mindist_c);
                    }
                }
                score_1 += mindist as f64;
            } else {
                for i in 0..input.m{
                    let mut mindist_c = usize::MAX;
                    let mut is_on = false;
                    ...
                    if is_on{
                        score_1 += 100.0/((1+c.dist(&(self.ans[i]+input.centers[i]))) as f64);
                    }
                }
            }
        }
...
impl Solver{
    fn new(input: &Input, n_candidates: usize) -> Self{
        ...
        let candidates = vec![];
        let mut valid = vec![HashSet::new(); input.m];
        for idx in 0..input.m{
            for i in 0..input.max_cs[idx].row{
                for j in 0..input.max_cs[idx].col{
                    valid[idx].insert(Coordinate::new(i, j));
                }
            }
        }
        let record = Record{q1: vec![], qd: vec![], a: vec![], oil_map: Map2d::new(vec![usize::MAX; input.n*input.n], input.n), oil_cnts: HashMap::new(), valid};
