...
impl Candidate{
        ...
        if self.score_1.0==record.q1.len(){
            for (i, from, to) in neighbor.moves.iter(){
                for cd in input.D[*i].iter(){
                    let c_pre = *from+*cd;
                    let c_new = *to+*cd;
                    if record.oil_map[c_pre]<self.V[c_pre]{
                        self.score_1.1 -= 1.0;
                    } else if record.oil_map[c_pre]!=usize::MAX{
                        self.score_1.1 += 1.0;
                    }
                    self.V[c_pre] -= 1;
                    if record.oil_map[c_new]<=self.V[c_new]{
                        self.score_1.1 += 1.0;
                    } else if record.oil_map[c_new]!=usize::MAX{
                        self.score_1.1 -= 1.0;
                    }
                    self.V[c_new] += 1;
                }
    ...
    fn get_score_1(&mut self, input: &Input, record: &Record) -> f64{
        ...
        let mut score_1 = self.score_1.1;
        let e = input.e;
        let q1i = self.score_1.0;
        for (c, v) in record.q1.iter().skip(q1i){
            score_1 += v.abs_diff(self.V[*c]) as f64;
        }
...
impl Solver{
    fn new(input: &Input, n_candidates: usize) -> Self{
        ...
        let candidates = vec![];
        let record = Record{q1: vec![], qd: vec![], a: vec![], oil_map: Map2d::new(vec![usize::MAX; input.n*input.n], input.n), oil_cnts: HashMap::new()};
    ...
    fn measure(&mut self, input: &Input, flg: usize, cs: Vec<Coordinate>)->usize{
        ...
        self.turn += 1;
        if flg==0{
            println!("q 1 {} {}", cs[0].row, cs[0].col);
            input!{
                from &mut source,
                resp: usize
            }
            self.measured.insert(cs[0]);
            self.record.q1.push((cs[0], resp));
            self.record.oil_map[cs[0]] = resp;
            if resp>0{
                self.record.oil_cnts.insert(cs[0], resp);
            }
