...
impl State{
    fn init_state(input: &Input) -> Self{
        ...
        let mut mean_dists = vec![0; input.m];
        for i in 0..input.m{
            mean_dists[i] = input.dists[input.m][i][0].0;
        }
        while appeared.len()<input.m{
            let mut best_score = 999999;
            let mut best_idx = 0;
            let mut best_charpos_idx = 0;
            for i in 0..input.m{
                if appeared.contains(&i){
                    continue;
                }
                let (dist, charpos_idx) = input.dists[last_idx][i][last_charpos_idx];
                let score = dist as i64 - mean_dists[i] as i64;
                if dist<best_score{
                    best_score = dist;
                    best_idx = i;
                    best_charpos_idx = charpos_idx;
                }
            }
            appeared.insert(best_idx);
            order.push(best_idx);
            last_idx = best_idx;
            last_charpos_idx = best_charpos_idx;
        }
        Self {order}
    }
    ...
    fn update(&mut self, params: &Neighbor){
        if params.neighbor_type==0{
            ...
            self.order = params.idxs[self.order.len()..].to_vec()
        }
    }
    ...
    fn get_neighbor(&mut self, input: &Input) -> Neighbor{
        ...
        if mode_flg<5{
            ...
            loop {
                ...
                let mut idxs = self.order.clone();
                if to<l{
                    idxs.append(&mut self.order[..to].to_vec());
                    idxs.append(&mut self.order[l..r].to_vec());
                    idxs.append(&mut self.order[to..l].to_vec());
                    idxs.append(&mut self.order[r..].to_vec());
                } else{
                    idxs.append(&mut self.order[..l].to_vec());
                    idxs.append(&mut self.order[r..to].to_vec());
                    idxs.append(&mut self.order[l..r].to_vec());
                    idxs.append(&mut self.order[to..].to_vec());

                }
                return Neighbor{neighbor_type: 2,  idxs}
            }
        }
    }
...
fn main() {
    ...
    solve(&input, &timer, 1.95);
}

fn solve(input: &Input, timer:&Instant, tl: f64){
    let mut init_state = State::init_state2(input);
