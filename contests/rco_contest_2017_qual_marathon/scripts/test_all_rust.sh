...
# cargo build --manifest-path ./Cargo.toml --bin rco_contest_2017_qual_a --release || exit 1

# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    ./target/debug/rco_contest_2017_qual_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    # ./target/release/rco_contest_2017_qual_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    cd ./tools
