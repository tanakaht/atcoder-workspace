        ...
        for i in 0..2*N*(N-1){
            for j in i+1..2*N*(N-1){
                for _ in 0..BREND_TEST_CNT{
                    ...
                    addj_scores[i][j] += ((seed.score_weighted(&weight) as f64)/BREND_TEST_CNT as f64);
                }
                if t>7{
                    addj_scores[i][j] = addj_scores[i][j].powf(2);
                }
...
impl Input {
    fn read_input() -> Self {
        ...
        Self { n, m, t, seeds: Seeds::new(seeds)}
    }
}
...
fn solve(input: &Input, timer:&Instant, tl: f64){
    ...
    for t in 0..T{
        ...
        let mut best_state = simanneal(input, init_state, timer, tl*((t+1) as f64/T as f64));
        best_state.print(input);
        seeds = Seeds::read_seeds();
    }
}
