...
fn main() {
    ...
    solve(&input, &timer, 1.98*1.0);
}

fn solve(input: &Input, timer:&Instant, tl: f64){
    let mut init_state = simanneal(input, State::init_state(input), timer, 0.2);
    let mut init_state2 = simanneal(input, State::init_state2(input), timer, 0.4);
    // let mut init_state2 = State::init_state2(input);
    if init_state.get_score(input)>init_state2.get_score(input){
