...
impl Dijkstra {
    fn new(G: &Vec<Vec<(usize, usize, usize)>>, start_node: usize, end_node: Option<usize>, removed_edge: &HashSet<usize>) -> Self {
        ...
        while let Some((d_, u, fr_u, fr_i)) = q.pop() {
            ...
            appeared[u] = true;
            if fr_u!=INF{
                parents[u] = Some((fr_u, fr_i));
                children[fr_u].push(u);
            }
        ...
        Self { dists, start_node, end_node, parents, children, total_dist }
    }
    ...
    fn recalculate(&mut self, input: &Input, removed_edge: &HashSet<usize>, added_edges: &Vec<usize>, poped_edges: &Vec<usize>){
        let mut q: BinaryHeap<(usize, usize, usize, usize)> = BinaryHeap::new();
        let mut appeared = vec![false; input.N];
        // popについてdistとqを定義
        // targetを取ってくる
        let mut target_node: HashSet<usize> = HashSet::new();
        for i in poped_edges.iter(){
            let mut q_dfs: BinaryHeap<usize>  = BinaryHeap::new();
            let (u, v, w) = input.UVW[*i];
            if let Some((pu_u, pu_i)) = self.parents[u]{
                if pu_u==v{
                    q_dfs.push(u);
                }
            }
            if let Some((pv_v, pv_i)) = self.parents[v]{
                if pv_v==u{
                    q_dfs.push(v);
                }
            }
            while let Some(node) = q_dfs.pop(){
                ...
                for node_ in self.children[node].iter(){
                    q_dfs.push(*node_);
                }
            }
        }
        ...
        while let Some((d_, u, fr_u, fr_i)) = q.pop() {
            ...
            appeared[u] = true;
            if fr_u!=INF{
                self.parents[u] = Some((fr_u, fr_i));
                self.children[fr_u].push(u);
            }
            for (v, c, i) in input.G[u].iter(){
                let d2 = d+c;
...
impl State {
    fn init(input: &Input, seed: u128) -> Self {
        ...
        let mut daily_dijkstra = vec![];
        let mut daily_score = vec![0; input.D];
        for d in 0..input.D{
...
fn climbing(input: &Input, init_state: State, timer: &Instant, limit: f64) -> State{
    ...
    while timer.elapsed().as_secs_f64()<limit{
        ...
        let aft_score_d1 = State::get_daily_score_with_sample_nodes(input, &state.assigns[d1], &state.sample_nodes);
        let aft_score_d2 = State::get_daily_score_with_sample_nodes(input, &state.assigns[d2], &state.sample_nodes);
