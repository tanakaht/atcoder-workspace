...
impl Dijkstra {
    fn new(G: &Vec<Vec<(usize, usize, usize)>>, start_node: usize, end_node: Option<usize>, removed_edge: &Vec<bool>) -> Self {
        let N = G.len();
    ...
    fn recalculate(&mut self, input: &Input, removed_edge: &Vec<bool>, unremoved_edge: &Vec<usize>, additional_remove_edge: &Vec<usize>){
        let mut q: BinaryHeap<(Reverse<usize>, usize)> = BinaryHeap::new();
        ...
        for i in additional_remove_edge.iter(){
            ...
            while let Some(node) = q_dfs.pop(){
                ...
                for (node_, c, i) in input.G[node].iter(){
                    if self.dists[node]+c==self.dists[*node_] && !removed_edge[*i]{
                        q_dfs.push(*node_);
                    }
                }
            }
        ...
        for u in target_node.iter(){
            for (v, c, i) in input.G[*u].iter(){
                ...
                if self.dists[*u]<=d || removed_edge[*i]{
                    continue;
                }
        ...
        while let Some((Reverse(d), u)) = q.pop() {
            ...
            for (v, c, i) in input.G[u].iter(){
                let d2 = d+c;
                if self.dists[*v]<=d2 || removed_edge[*i]{
                    continue;
                }
...
impl Input {
    fn read_input() -> Self {
        ...
        for (i, (start_node, end_node, w)) in UVW.iter().enumerate(){
            let mut removed_edge = vec![false; M];
            removed_edge[i] = true;
            let dijkstra = Dijkstra::new(&G, *start_node, Some(*end_node), &removed_edge);
        ...
        Self { N, M, D, K, UVW, XY, G, depends_pair }
    }
}
...
impl State {
    fn init(input: &Input, seed: u128) -> Self {
        ...
        // removed_edge_flg
        let mut removed_edge_flg = vec![vec![false; input.M]; input.D];
        for (d, ss) in assigns.iter().enumerate(){
            for i in ss.iter(){
                removed_edge_flg[d][*i] = true;
            }
        }
        // ansを反映
        ...
        let sample_nodes: HashSet<usize> = HashSet::from_iter(sample_nodes_);
        ...
        for d in 0..input.D{
            ...
            for u in sample_nodes.iter(){
                let dijkstra = Dijkstra::new(&input.G, *u, None, &removed_edge_flg[d]);
                daily_score[d] += dijkstra.total_dist;
                dijkstras.push(dijkstra);
            }
            daily_dijkstra.push(dijkstras);
        }
        Self { assigns, removed_edge_flg, ans, daily_score, daily_dijkstra }
    }
            ...
            for _ in 0..100{
                ...
                loop {
                    ...
                    for (v, w, i) in input.G[cur].iter(){
                        if self.removed_edge_flg[d1][*i] && rng.gen::<usize>()%100<80 && !us1.contains(i){
                            cur = *v;
                            us1.push(*i);
                            found = true;
                            break;
                ...
                loop {
                    let mut found = false;
                    for (v, w, i) in input.G[cur].iter(){
                        if self.removed_edge_flg[d2][*i] && rng.gen::<usize>()%100<80 && !us2.contains(i){
                            cur = *v;
                            us2.push(*i);
                            found = true;
                            break;
    ...
    fn update(&mut self, input: &Input, params: (usize, Vec<usize>, usize, Vec<usize>)){
        ...
        for u1 in us1.iter(){
            ...
            self.removed_edge_flg[d1][*u1] = false;
            self.removed_edge_flg[d2][*u1] = true;
            self.ans[*u1] = d2;
        }
        for u2 in us2.iter(){
            self.assigns[d2].remove(u2);
            self.assigns[d1].insert(*u2);
            self.removed_edge_flg[d2][*u2] = false;
            self.removed_edge_flg[d1][*u2] = true;
            self.ans[*u2] = d1;
        }
        self.daily_score[d1] = 0;
        self.daily_score[d2] = 0;
        for dijkstra in self.daily_dijkstra[d1].iter_mut(){
            dijkstra.recalculate(input, &self.removed_edge_flg[d1], &us1, &us2);
            self.daily_score[d1] += dijkstra.total_dist;
        }
        for dijkstra in self.daily_dijkstra[d2].iter_mut(){
            dijkstra.recalculate(input, &self.removed_edge_flg[d2], &us2, &us1);
