...
class State:
        ...
        for i in range(100):
            ...
            for i, (u, v, w) in random.sample(list(enumerate(UVW)), M):
                ...
                for d in range(D):
                    ...
                    if depend_cnt!=0:
                        continue
                    metrics = (len(removed_edges[d]), max(node_removed_edge_cnt[d][u], node_removed_edge_cnt[d][v]))
    ...
    def get_neighbor_with_score(self, *args):
        ...
        if params[0]=="swap":
            _, i, x, j, y = params
            sampling_point = set([UVW[x][0], UVW[x][1], UVW[y][1], UVW[y][1]]) # +[randint(0, N) for _ in range(6)])
            ...
            _, i, x, j = params
            sampling_point = set([UVW[x][0], UVW[x][1]]) # +[randint(0, N) for _ in range(8)])
            # sampling_point = set(range(N))
            pre_score_i = self.get_score_with_sampleing_point(self.removed_edges[i], sampling_point)
            pre_score_j = self.get_score_with_sampleing_point(self.removed_edges[j], sampling_point)
    ...
    def get_score(self, **params):
        return sum([self.get_daily_score(day) for day in range(D)])
