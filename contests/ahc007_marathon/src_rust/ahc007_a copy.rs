...
impl Input {
    ...
    fn min_dist(&mut self, a: usize, b: usize) -> usize{
        let mut groups = self.uf.groups();
        let mut dists = vec![usize::MAX; groups.len()];
        let mut gidx2idx = groups.iter().enumerate().map(|(i, g)| (self.uf.find(g[0]), i)).collect::<HashMap<_, _>>();
        let mut s = gidx2idx[&self.uf.find(a)];
        let mut t = gidx2idx[&self.uf.find(b)];
        let mut que = BinaryHeap::new();
        que.push((Reverse(0), s));
        dists[s] = 0;
        while let Some((Reverse(d), u_idx)) = que.pop(){
            if u_idx==t{
                return d;
            }
            if dists[u_idx]<d{
            ...
            for &u in groups[u_idx].iter(){
                for &(l, v) in self.graph[u].iter(){
                    let d_ = d+l;
                    let v_idx = gidx2idx[&self.uf.find(v)];
                    if dists[v_idx]>d_{
                        dists[v_idx] = d_;
                        que.push((Reverse(d_), v_idx));
                    }
                }
            }
        }
...
fn should_accept(input: &Input, l_act: usize, timer:&Instant, tl: f64) -> bool{
    ...
    for edges in input.sample_edges.iter(){
        if total_cnt>=10 && timer.elapsed().as_secs_f64()>tl{
            break;
        }
...
fn solve(input: &mut Input, timer:&Instant, tl: f64){
    ...
    for t in 0..M{
        ...
        // let flg = should_accept(input, l_act, timer, start_time+1.5*(tl-start_time)*(t+1) as f64/M as f64);
        let flg = should_accept(input, l_act, timer, tl);
        if flg{
            println!("1");
            input.accept_edge();
        } else {
