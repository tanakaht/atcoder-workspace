...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/atcoder11live/f62fc84_windows.zip).
...
- `--n=20` Fix the $n$ value of the generated inputs to the specified one.
- `--p=0.01` Fix the $p$ value of the generated inputs to the specified one.
...
You can also use a [web visualizer](https://img.atcoder.jp/atcoder11live/f62fc84.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/atcoder11live/f62fc84_windows.zip)も用意してあります。
...
- `--n=20` 生成される入力の $n$ の値を指定されたものに固定
- `--p=0.01` 生成される入力の $p$ の値を指定されたものに固定
...
より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/atcoder11live/f62fc84.html?lang=ja)も利用可能です。
