...
use rand::prelude::*;
use svg::node::{
    element::{path::Data, Group, Path, Polygon, Rectangle, Symbol, Title, Use},
    Text,
};
...
pub type Output = Vec<(usize, usize)>;
...
impl std::fmt::Display for Input {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let n = self.bs.len();
        writeln!(f, "{}", n)?;
        writeln!(f, "{} {}", self.si, self.sj)?;
        for i in 0..n {
            writeln!(f, "{}", self.bs[i].iter().collect::<String>())?;
        }
        Ok(())
    }
}
...
pub fn parse_output(input: &Input, f: &str) -> Result<Output, String> {
    ...
    let mut out = vec![];
    let m = read(tokens.next(), 0, n * n)?;
    for _ in 0..m {
        out.push((read(tokens.next(), 0, n - 1)?, read(tokens.next(), 0, n - 1)?));
    }
...
pub fn gen(seed: u64, fix_n: Option<usize>, fix_p: Option<f64>) -> Input {
    let mut rng = rand_chacha::ChaCha20Rng::seed_from_u64(seed ^ 25);
    let mut n = rng.gen_range(20, 51) as usize;
    if let Some(n2) = fix_n {
        n = n2;
    }
    let mut p = rng.gen_range(0.01, 0.1);
    if let Some(p2) = fix_p {
        p = p2;
    }
    let si = rng.gen_range(0, n as i32) as usize;
    let sj = rng.gen_range(0, n as i32) as usize;
    let mut bs = mat!['.'; n; n];
    for i in 0..n {
        for j in 0..n {
            if (i, j) != (si, sj) && rng.gen_bool(p) {
                bs[i][j] = '#';
            }
        }
    }
...
pub struct State {
    ...
    pub bs: Vec<Vec<char>>,
    pub err: String,
    pub pos: Vec<(usize, usize, usize)>,
}

impl State {
    pub fn new(input: &Input, out: &Output) -> State {
        let n = input.bs.len();
        let mut bs = input.bs.clone();
        let mut err = String::new();
        for &(i, j) in out {
            if bs[i][j] != '.' {
                err = format!("({}, {}) already has an obstacle.", i, j);
            } else if (i, j) == (input.si, input.sj) {
                err = format!("You cannot place an obstacle in the initial position.");
            }
            bs[i][j] = 'o';
        }
        ...
        let score = if err.len() > 0 {
            0
        } else {
            (1e6 * t as f64 / (4.0 * empty as f64)).round() as i64
        };
        State { score, bs, err, pos }
    }
}
...
pub fn vis(state: &State, t: usize) -> (i64, String, String) {
    ...
    for i in 0..n {
        for j in 0..n {
            let color = if state.bs[i][j] == '#' {
                "gray"
            } else if state.bs[i][j] == 'o' {
                "saddlebrown"
            } else {
                "white"
            };
            doc = doc.add(
                Group::new()
                    ...
                    .add(rect(j * D, i * D, D, D, color).set("stroke", "gray").set("stroke-width", 1)),
            );
        }
    }
    let mut data = Data::new().move_to((state.pos[0].1 * D + D / 2, state.pos[0].0 * D + D / 2));
    for i in 1..=t {
        data = data.line_to((state.pos[i].1 * D + D / 2, state.pos[i].0 * D + D / 2));
        let cx = state.pos[i - 1].1 * D + D / 2;
        let cy = state.pos[i - 1].0 * D + D / 2;
        if state.pos[i - 1].0 < state.pos[i].0 {
            doc = doc.add(
                Polygon::new()
                    .set(
                        "points",
        ...
        } else if state.pos[i - 1].0 > state.pos[i].0 {
            doc = doc.add(
                Polygon::new()
                    .set(
                        "points",
        ...
        } else if state.pos[i - 1].1 < state.pos[i].1 {
            doc = doc.add(
                Polygon::new()
                    .set(
                        "points",
    ...
    let path = Path::new()
        ...
        .set("d", data);
    doc = doc.add(path);
    let (pi, pj, dir) = state.pos[t as usize];
    let mut car = Use::new().set("href", "#car").set("width", 2 * D).set("height", 2 * D);
    let rotate = if dir == 0 {
        180
    } else if dir == 1 {
        270
    ...
    car = car
        .set(
            ...
            format!(
                "rotate({}) translate({} {})",
                rotate,
                (pj * D) as i32 - D as i32 / 2,
                (pi * D) as i32 - D as i32 / 2
            ),
        )
        .set("transform-origin", format!("{} {}", pj * D + D / 2, pi * D + D / 2));
    doc = doc.add(
        Group::new()
            .add(Title::new().add(Text::new(format!("({}, {})", pi, pj))))
            .add(car),
    );
    (state.score, state.err.clone(), doc.to_string())
}
