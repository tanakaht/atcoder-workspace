...
problem_name=$1
cargo build --manifest-path ./Cargo.toml --bin atcoder11live_a || exit 1
# cargo build --manifest-path ./Cargo.toml --bin atcoder11live_a --release || exit 1
# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    ./target/debug/atcoder11live_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    # ./target/release/atcoder11live_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    cd ./tools
