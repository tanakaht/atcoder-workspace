...
problem_name=$1
# cargo build --manifest-path ./Cargo.toml --bin ahc026_a || exit 1
...
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    # ./target/debug/ahc026_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    ./target/release/ahc026_a  < "./tools/in/${f}.txt" > "./tools/out/${f}.txt" 2> "./tools/err/${f}.txt"
    cd ./tools
