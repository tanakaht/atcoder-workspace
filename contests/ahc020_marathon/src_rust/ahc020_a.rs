...
impl Input {
    fn read_input() -> Self {
        ...
        let mut G: Vec<Vec<(usize, usize, usize)>> = vec![vec![]; 100];
...
impl State{
    ...
    fn get_mst(&mut self, input: &Input) -> Vec<bool>{
        ...
        let mut used_tower_set:HashSet<usize> = HashSet::new();
        for (i, flg) in self.used_tower.iter().enumerate(){
            if *flg{
                used_tower_set.insert(i);
            }
        }
        ...
        let mut children: Vec<Vec<usize>> = vec![Vec::new(); input.N];
        let mut parents= vec![input.N; input.N];
        let mut stack: Vec<(usize, usize)> = vec![(0, input.N)];
        let mut tree_ord = vec![];
        while stack.len()>0 {
            let (u, p) = stack.pop().unwrap();
            tree_ord.push(u);
            for (v, w, i) in input.G[u].iter() {
                if *v != p && used_edge[*i] {
                    parents[*v] = u;
                    children[u].push(*v);
                    stack.push((*v, u));
        ...
        let mut removed_edge: HashSet<(usize, usize)> = HashSet::new();
        for x in tree_ord.iter().rev(){
            let mut flg = true;
            for v in children[*x].iter(){
                if unused.contains(v){
                    removed_edge.insert((*x, *v));
                    removed_edge.insert((*v, *x));
                } else {
                    flg = false;
                }
            }
            if (flg && !used_tower_set.contains(x)){
                unused.insert(*x);
            }
        }
        for (u, v, w, i) in input.UVWi.iter(){
            if used_edge[*i] && removed_edge.contains(&(*u, *v)){
                used_edge[*i] = false;
            }
        }
        used_edge
    ...
    fn get_neighbor(&mut self, input: &Input) -> Vec<(usize, bool)>{
        let mut rng = rand::thread_rng();
        let mode_flg = rng.gen::<usize>()%100;
        let mut ret = vec![];
        if mode_flg < 50{
            let i = rng.gen::<usize>()%self.used_tower.len();
            ret.push((i, !self.used_tower[i]));
        } else {
            let i = rng.gen::<usize>()%self.used_tower.len();
            ret.push((i, !self.used_tower[i]));
            let mut i_rep: usize = 0;
            let mut i_rep_d: i64 = 1<<32;
            for j in 1..self.used_tower.len(){
                if self.used_tower[i]==self.used_tower[j]{
                    continue;
                }
                let d = input.Pos_tower[i].dist(&input.Pos_tower[j]);
                if i_rep_d > d{
                    i_rep = j;
                    i_rep_d = d;
                }
            }
            ret.push((i_rep, !self.used_tower[i_rep]));
        }
        if ret.len()==0{
            return self.get_neighbor(input);
        }
        ret
    }
...
fn simanneal(input: &Input, init_state: State, timer: &Instant, limit: f64) -> State{
    ...
    loop {
        ...
        let params = new_state.get_neighbor(input);
        new_state.update(&params);
        let new_score = new_state.get_score(input);
