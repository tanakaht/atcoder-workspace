...
struct State{
    map: Map2d<usize>,
    white_neighbors: HashSet<Coordinate>,
}
...
impl State{
    fn init_state(input: &Input) -> Self{
        ...
        for i in 1..51{
            white_neighbors.insert(Coordinate { row: 1, col: i });
            white_neighbors.insert(Coordinate { row: 50, col: i });
            white_neighbors.insert(Coordinate { row: i, col: 1 });
            white_neighbors.insert(Coordinate { row: i, col: 50 });
        }
        Self {map: input.map.clone(), white_neighbors}
    }
    ...
    fn update(&mut self, params: &NeighborParam){
        for (c, before_color, after_color) in &params.x{
            ...
            if *before_color==0{
                for c_ in c.neighbor4(){
                    let mut flg = false;
                    for c__ in c_.neighbor4(){
                        if self.map[c__]==0{
                            flg = true;
                            break;
                        }
                    ...
                    if flg{
                        if self.map[c_]!=0{
                            self.white_neighbors.insert(c_);
                        }
                    } else {
                        self.white_neighbors.remove(&c_);
                    }
                }
            }
            if *after_color==0{
