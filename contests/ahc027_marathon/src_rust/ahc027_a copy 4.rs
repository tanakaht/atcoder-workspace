...
impl State{
    ...
    fn update(&mut self, params: &Neighbor){
        ...
        for &idx in params.old_pathes.iter(){
            self.cnts[idx] -= 1;
        }
    }
        ...
        if flg<FLG_IDX[1]{
            ...
            for (nidx, nidx_) in neighbor_pairs.iter(){
                if input.neighbors[*nidx].contains(nidx_){
                    return Neighbor{fr, to, new_pathes: vec![idx, *nidx, *nidx_], old_pathes: vec![idx], neighbortype: 1};
                }
            }
            ...
            loop{
                ...
                loop {
                    ...
                    return Neighbor{fr, to, new_pathes: vec![idx, nidx, idx], old_pathes: vec![idx], neighbortype: 2};
                }
            }
            ...
            return Neighbor{fr: fr+1, to: to+1, new_pathes, old_pathes: self.pathes[fr+1..to+1].to_vec(), neighbortype: 3};
            ...
            return Neighbor{fr: fr+1, to: to+1, new_pathes, old_pathes: self.pathes[fr+1..to+1].to_vec(), neighbortype: 4};
            ...
            for to in fr+2..self.pathes.len(){
                ...
                if input.neighbors[idx].contains(&idx_){
                    return Neighbor{fr: fr+1, to, new_pathes: vec![], old_pathes: self.pathes[fr+1..to].to_vec(), neighbortype: 5};
                }
            }
            ...
            let idx = self.pathes[fr];
            if self.cnts[idx]<=1{
                return Neighbor { fr: 0, to: 0, new_pathes: vec![], old_pathes: vec![], neighbortype: 0};
                // continue;
            }
            // let mut idxs = (fr+2..self.pathes.len()).collect::<Vec<_>>();
            // idxs.shuffle(&mut rng);
            for to in fr+2..self.pathes.len(){
                ...
                if idx==idx_{
                    ...
                    new_pathes.reverse();
                    return Neighbor{fr: fr+1, to, new_pathes, old_pathes: self.pathes[fr+1..to].to_vec(), neighbortype: 6};
                }
            }
            ...
            let idx = self.pathes[fr];
            let idx_ = self.pathes[to];
            let midx = input.cluster_points.iter().choose(&mut rng).unwrap().clone();
            ...
            return Neighbor{fr: fr+1, to: to+1, new_pathes, old_pathes: self.pathes[fr+1..to+1].to_vec(), neighbortype: 7};
            ...
            for fr2 in 0..self.pathes.len()-2{
                ...
                for to2 in fr2+2..cmp::min(self.pathes.len(), fr2+100){
                    ...
                    if let Some(&to) = candidates.get(&idx2_){
                        ...
                        return Neighbor{fr: fr_min, to: to_max, new_pathes, old_pathes: self.pathes[fr_min..to_max].to_vec(), neighbortype: 8}
                    }
                }
            }
            ...
            for to in fr+2..self.pathes.len(){
                ...
                if idx==idx_{
                    ...
                    new_pathes.reverse();
                    return Neighbor{fr: fr-fr_b, to: to+to_f+1, new_pathes, old_pathes: self.pathes[fr-fr_b..to+to_f+1].to_vec(), neighbortype: 9};
                }
            }
            ...
            if fr<fr2{
                return Neighbor{fr, to: fr2, new_pathes: [self.pathes[to..fr2].to_vec(), self.pathes[fr..to].to_vec()].concat(), old_pathes: self.pathes[fr..fr2].to_vec(), neighbortype: 10};
            } else {
                return Neighbor{fr: fr2, to, new_pathes: [self.pathes[fr..to].to_vec(), self.pathes[fr2..fr].to_vec()].concat(), old_pathes: self.pathes[fr2..to].to_vec(), neighbortype: 10};
            ...
            let (fr2, to2) = (turns[fr2_idx], turns[fr2_idx+1]);
            let new_pathes = [self.pathes[fr2..to2].to_vec(), self.pathes[to..fr2].to_vec(), self.pathes[fr..to].to_vec()].concat();
            return Neighbor{fr: fr2, to: fr2, new_pathes: self.pathes[fr..to].to_vec(), old_pathes: vec![], neighbortype: 10};
        }
        Neighbor { fr: 0, to: 0, new_pathes: vec![], old_pathes: vec![], neighbortype: 0}
    }
    ...
    fn get_score_old2(&self, input: &Input) -> f64{
        let l = self.pathes.len()-1;
