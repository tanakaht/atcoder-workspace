...
impl State{
    fn init_state(input: &Input) -> Self{
        ...
        for loop_i in 0..10{
            ...
            while unvisited.len()>0{
                ...
                let mut q = VecDeque::new();
                q.push_front(cur);
                let mut appeared = vec![vec![false; input.n]; input.n];
                while !q.is_empty(){
                    ...
                    //     continue;
                    // }
                    for &(h, w) in input.neighbors[to.0][to.1].iter(){
                        if !appeared[h][w]{
                            q.push_back((h, w));
                            appeared[h][w] = true;
                        }
                ...
                let mut idxs = input.neighbors[cur.0][cur.1].clone();
                // idxs.shuffle(&mut rng);
                // idxs = [&idxs[loop_i%idxs.len()..], &idxs[..loop_i%idxs.len()]].concat();
                for &(h, w) in idxs.iter(){
                    if unvisited.contains(&(h, w)){
                        to = (h, w);
                        break;
                    }
                }
                while cur!=to{
                    let (h, w) = cur;
                    let (h_, w_) = to;
                    let next = input.mindistmap[h_][w_][h][w].choose(&mut rng).unwrap().clone();
    ...
    fn get_neighbor(&mut self, input: &Input) -> Neighbor{
        ...
        if flg<20{
            ...
            let (x, y) = self.pathes[fr];
            let (x_, y_) = self.pathes[to];
            let (mx, my) = (rng.gen::<usize>()%input.n,rng.gen::<usize>()%input.n);
            let mut new_pathes = vec![];
            let mut cur = (x, y);
            while cur!=(mx, my){
                let (h, w) = cur;
                let next = input.mindistmap[mx][my][h][w].choose(&mut rng).unwrap().clone();
                new_pathes.push(next);
                cur = next;
            }
            while cur!=(x_, y_){
                let (h, w) = cur;
                let next = input.mindistmap[x_][y_][h][w].choose(&mut rng).unwrap().clone();
                new_pathes.push(next);
                cur = next;
            }
            return Neighbor{fr: fr+1, to: to+1, new_pathes, old_pathes: self.pathes[fr+1..to+1].to_vec()};
        } else if flg<60{
