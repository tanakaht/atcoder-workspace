    ...
    fn optimize_fixed_length_iter(&mut self, input: &Input, timer: &Instant, tl: f64){
        ...
        loop {
            ...
            for fr in 0..self.pathes.len()-6{
                ...
                // eprintln!("cand: {}", candidate.len());
                if candidate.len()==1{
                    continue;
                }
                for (i, idx__) in self.pathes[fr+1..fr+6].iter().enumerate(){
...
fn solve(input: &Input, timer:&Instant, tl: f64){
    ...
    let mut best_state = simanneal(input, init_state, timer, tl-0.1);
