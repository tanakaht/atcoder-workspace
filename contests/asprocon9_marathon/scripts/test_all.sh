...
cp ./src/asprocon9_a.py ./dev/solve_tmp.py
# インタラクティブ処理の関数
f1(){
    #f=$(printf "%04d\n" "${1}")
    f=${1}
    # ./tools/judge "pypy3 ./dev/solve_tmp.py" < "./tools/in/${f}.txt" > './tools/out/${f}.txt'
    score="$(./tools/judge "pypy3 ./dev/solve_tmp.py" < "./tools/in/${f}.txt" 2>&1 > /dev/null | awk '{ print $3 }')"
    # pypy3 ./dev/solve_tmp.py < "./tools/in/${f}.txt" > "./tools/out/${f}.txt"
    # cd ./tools
    # score=$(cargo run --release --bin vis "./in/${f}.txt" "./out/${f}.txt" 2>/dev/null | awk '{ print $2 }')
    # cd ..
